...
inline void softmax(float *io, uint32_t size) {
  ...
  // get frac for each value and store it back
  for(uint32_t i=0; i < cnt_block; ++i) {
    
    float32_t one_div_sum = 1.0f / sum;

    load_data = vld1q_f32(io);
    result_data = vmulq_n_f32(exp_ps(load_data), one_div_sum);
    result_data = log_ps(result_data);
    //store it back
